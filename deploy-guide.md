# Деплой А-Недвижимость на хостинг — пошаговая инструкция

Архив `realty-portal-source.zip` (852 КБ) уже лежит у вас в папке `Realty_portal`. Дальше — по шагам: сначала код попадает на GitHub, потом с GitHub — на хостинг Timeweb.
...
В панели Timeweb, в настройках домена/сайта, найдите поле «Директория сайта» или «Document root» и укажите:
```
...
!! Команда в SSH консоли для смены папки для хостинга Timeweb:
```bash
ln -s /home/u/user/myproject/public /home/u/user/public_html
```
(сначала удаляем public_html из директории сайта, а потом вводим команду выше)
