...
<section>
    ...
    <form wire:submit="updateProfileInformation" class="mt-6 space-y-6">
        ...
        <div>
            ...
            {{-- ВАЖНО: значение x-mask должно быть "голым" паттерном без JS-кавычек.
                 Плагин Alpine mask использует НЕ функцию/динамический режим (нет модификатора
                 :dynamic), поэтому берёт expression как есть, символ в символ, в качестве
                 шаблона маски. Если обернуть паттерн в одинарные кавычки как JS-строку
                 ('+7 (999) 999-99-99'), эти кавычки становятся ЧАСТЬЮ шаблона: ведущая
                 кавычка выводится как обычный литеральный символ маски (перед номером),
                 а замыкающая — почти никогда не достигается, потому что цикл форматирования
                 останавливается, как только заканчиваются введённые цифры. Именно это раньше
                 давало апостроф перед номером телефона и ломало серверную валидацию
                 (значение переставало соответствовать регулярному выражению RussianPhone). --}}
